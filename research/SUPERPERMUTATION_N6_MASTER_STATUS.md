...
| 94 (Claude branch) | **Path connectivity: layers A/B are cheap, layer C is not, 228 provisional closures, ledger held at 5,030.** Round 93's Hall condition assigns predecessors injectively but ignores that a continuation is one time-ordered walk. On the obligation digraph (empty hexagons plus the fragment, edge x->y iff some entry candidate of x reaches some entry candidate of y by sigma^ell + joint) rooted at the current pass, in-degree 1 and out-degree <= 1 force the selected edges to split into one rooted path plus cycles. **Layer A** (root reachability) fails 5,344 pairs; **layer B** (closed subset with no external predecessor), implemented independently, coincides with A on **every** pair, so the two are the same condition and are not counted twice; **layer B2** (a single path has exactly one end, hence >= 2 sink SCCs, >= 2 zero-out-degree obligations, or a zero-out-degree root are impossible) fails 1,796 pairs. Together they close **228 of 5,030 with 0 UNKNOWN in 24 s**. **Layer C** (Hall plus rooted connectivity, equivalent here to a rooted Hamiltonian path) is **not cheap**: a 60-state pilot capped on **427 of 427 pairs** in 146 s, and a cap is never UNSAT, so its payoff is 0. Positive controls on realized history pass: at most one registered port per state is unreachable from another registered port (the initial word), with no state having two. **But an asymmetry is unresolved**: states with r=1 (already-repaired fragment, two registered ports in one hexagon) are rejected at **37.8%** versus **0.7%** for r=0, and this is *not* explained by cover counts since r=1 states have more passing covers (median 7 vs 3). That class is exactly the one the round instructions singled out for testing, so the 228 closures are withheld from the ledger pending an explanation. Unique-cover split: 20 of 746 (2.7%) versus 208 of 4,284 (4.9%) | this branch, `research/RR_PATH_CONNECTIVITY_CLAUDE.md`, `outputs/rr_path_connectivity_ledger.jsonl.gz` |
| 93c (Claude branch) | **Hall audit artifact repair: no new theorem; the counts now replay from the archive alone.** Codex found the Hall necessary condition and the registration semantics sound but could not reproduce 1,366 / 5,030 because the Round-93 ledger lacked per-state and per-cover certificates. `outputs/rr_port_path_hall_archive/` now carries a 720-word pinned geometry table (joint targets for every ell = 0..5), literal rows for all **6,396** states (p, hex_masks, orbit_masks, F/S/H, O/P/D, c/r/K/b, fragment occupancy and repair entry), **all 90,396** Round-92 passing cover sets per state with duplicate derivations removed and **no first-success truncation**, the Hall result and Koenig deficient subset for every state-cover pair, complete matchings for the **5,030** SAT states, certificates for **every** cover of the **1,366** UNSAT states, and a SCHEMA.md giving the deterministic reconstruction rules. A verifier that uses only the standard library and imports none of this repository's search code validates every state row (masks, P, O, D = 5O - P, singleton current hexagon, 121 - P = empty + fragment), rebuilds every Hall graph from scratch, and reproduces **SAT 5,030 / UNSAT 1,366 with zero stored-vs-recomputed mismatches, zero bad SAT witnesses and zero bad UNSAT certificates**. Two interpretation corrections are recorded rather than deleted: the claim that **1,760 states have exactly one Hall-passing cover is retracted** (the implementation stopped at the first passing cover; complete enumeration gives 0:1366, 1:746, 2-5:2791, 6-20:1340, >20:153), and the deficit histogram {1:812, ...} was the last evaluated cover per state, the meaningful per-state minimum being {1:1206, 2:156, 3:4}. The `14fda65d` example replays from the archive alone and exhibits the fact Codex lacked. The audited ledger stays **6,396**; Round 93 is marked HALL NECESSITY SOUND / STATE COUNTS CLAUDE-REPRODUCED / INDEPENDENT COUNT AUDIT PENDING | this branch, `research/RR_PORT_PATH_HALL_AUDIT_CLAUDE.md`, `outputs/rr_port_path_hall_archive/SCHEMA.md` |
| 93 (Claude branch) | **Path-ordered port registration: one entry window per hexagon; 1,366 closed pending audit (audited baseline stays 6,396).** The 64 unique-cover states were re-decided on cover *sets* (64/64, no correction) and classified completely: **all 64 admit a G5-only realization** (36 G5-only, 28 both), so short structure is irrelevant for them. Reading `extend()` directly: rotations register nothing and a joint registers **exactly one** port, so a pass consumes one hexagon and registers only its entry window, while the other five windows are consumed as rotation interior and can **never** be registered. Across the archive the registered-port count per hexagon is empty 0 / passed 1 / **passed 2 exactly 710** (the already-repaired fragments) / current 1 / fragment 1, with zero exceptions. Hence every remaining empty hexagon must be entered exactly once at a word of the final 25 orbits, the next entry is `joint(sigma^5(u))`, and each pass fires once -- a Hamiltonian-path condition whose **bipartite-matching relaxation** (Hall violation only) is used for closure, with no caps or timeouts. Typically 105-106 of the 107 empty hexagons contain exactly one final-orbit word, so entries are effectively forced. Four positive controls pass 6,657/6,657 on **realized** history. A literal dissection shows hexagon 101 of state `14fda65d` has a single final-orbit word whose four predecessors are two non-selected orbits and two already-visited words, so it can never be entered. Applying the joint and path conditions together over every valid cover: 13 of the 64, and **1,366 of 6,396 (SAT 5,030, UNKNOWN 0, 235 s)**, with deficits {1:812, 2:381, 3:144, 4:28, 5:1}. Self-correction: an initial '72% rejection' figure tested only one preserved witness cover per state and is not a state closure. Survivors' passing-cover counts are {1: 1760, 2-5: 1876, 6-20: 1316, >20: 78}, so the unique-cover target grows from 64 to 1,760 | this branch, `research/RR_PORT_PATH_CLAUDE.md`, `outputs/rr_port_path_ledger.jsonl.gz` |
| 92 (Claude branch) | **Fragment-local short opportunity: 186 closed, residual 6,396, and the short-edge family is exhausted.** With SHORT-PASS LOCALITY and the four former boundary witnesses now Codex-audited, the baseline is 6,582. Rounds 88/90 modelled a short exception as an arbitrary edge of `Gshort \ G5`; this round replaces it with the state's own **SHORT_LOCAL(s)** -- the openings reachable from the fragment's unique unvisited-run entry after `ell = 5 - c_f` rotations and one of the four joints. Census over the archive: budget 1 for 5,947 and 0 for 710; `c_f` spans 1-5 (**`c_f = 5` gives `ell = 0` in 1,487 states**, exactly the case Codex flagged); every budget-1 state has **exactly one source port** and **three distinct target orbits, of which one or two are fresh** -- no state has zero fresh local targets. The set is kept an over-approximation: `w2` joints are retained despite the blocked-w2 lemma, landing phases do not narrow candidates, and the theorem is NOT strengthened to 'the entry mask is always partial'. **Controls ran first**: the same pipeline reproduces 13 closures under a uniform budget of 2 (Round 85's SID set) and 71 under the state-dependent budget with generic Gshort (Round 90's set), both SID-for-SID, before SHORT_LOCAL was used for anything. The sweep over 6,582 gives **SAT 6,396 / UNSAT 186 / UNKNOWN 0 in 33 s** -- closures at c=2 (105), c=3 (78), c=4 (3), roots short_ell2 78 / short_ell1 61 / short_ell3 47, **all of them budget-1 states**. Attribution is unambiguous: **186 of 186** closures are because the generic model's short target lies outside SHORT_LOCAL(s); the payoff is entirely the target restriction, while the sound shared-port rule (the repair port cannot also source a G5 opening) adds none. The survivor diagnostic counted, completely, how many covers pass the joint condition: {1: 64, 2-5: 1335, 6-20: 3782, >20: 1215} -- and **no survivor has any passing cover that uses the short opportunity at all**, so tightening SHORT_LOCAL further cannot close anything. The next boundary set is the **64 states with a unique passing cover**, all short-free | this branch, `research/RR_SHORT_LOCAL_CLAUDE.md`, `outputs/rr_short_local_ledger.jsonl.gz` |
| 90c (Claude branch) | **Codex audit accepted: theorem confirmed, four-state closure retracted; audited residual 6,586.** Codex independently reproduced the corrected state-dependent short budget `1 - r` (archive 6,657: budget 1 -> 5,947, budget 0 -> 710, no exceptions), the literal current-hexagon fact, the uniform-budget-2 control closing exactly Round 85's 13 SIDs, and the corrected budget closing 71 in total -- hence **58 new closures, not 62**, and residual **6,586, not 6,582**. The subclaim 'all 19 unique-cover states are UNSAT' is **REFUTED**: 15 UNSAT, 4 SAT. The four survivors (`ed0e44ff…`, `6afa15d9…`, `afb08178…`, `b3cd052f…`) have minimum required short budget 1 equal to the available budget 1, with an explicit witness -- unique cover, G5 generation order, injective source-port assignment, one legal short exception -- and are preserved as adversarial boundary witnesses in `outputs/rr_boundary_witnesses_claude.json`. Wording also corrected: there are not always exactly two partial hexagons; at r=0 the fragment and the current hexagon are partial, at r=1 only the current one. The autopsy then answered why Round 90 and Codex disagree: Round 90's M2 fixed not only the *number* of short edges but their *place*. Enumerating every one-short witness (398,976-400,001 per state, one complete) shows the short edge departs from 24-30 distinct ports and **never from the fragment hexagon** -- 336k-356k of them start in a hexagon that will be empty at that time (departing short there is an abandonment, F=2 > TARGET_F), 40k-67k start in a hexagon that will be full (no unvisited start window exists), and 15,360 in one state start in the current hexagon whose only pass the engine forces to ell=5. That yields the candidate constraint **SHORT-PASS LOCALITY** -- an ell<5 edge must depart from a hexagon that is partially visited at that moment, which under F=1 is the fragment alone, fixing its start window and rotation length -- verified exhaustively at the rotation level (720 words x 6 lengths, 0 violations) but **left pending Codex audit and deliberately not used to move the ledger**. A bounded headroom diagnostic (200 random states, node cap 150k) gives SAT 193 / UNSAT 2 / UNKNOWN 5, so the constraint is worth about 1% more closures if it survives audit. Separately, the '121 of 125 final ports' observation is derived to be **not a new coordinate**: D == 5*O - P holds on all 6,657 and TARGET_D = 4, so it is exactly the D coordinate the engine already prunes on; only the *path* version would be new | this branch, `research/RR_BOUNDARY_WITNESSES_CLAUDE.md`, `outputs/rr_boundary_witnesses_claude.json` |
| 90 (Claude branch) | **Single short pass theorem: the ell<5 budget is 1 or 0, not 2 -- **58** states closed (corrected in Round 90c; 62 was wrong), residual 6,644 -> **6,586**.** The autopsy of Round 89's 19 unique-cover states found the error one level below the joint condition, in the budget it spends. In the literal checkpoint states **all 6,657 residual states have exactly one visited cell in the current hexagon** and F = 1 = TARGET_F, so no future abandonment is legal; since a departure at w requires sigma(w) already visited, the current pass is forced to ell = 5. With 120 hexagons, P = 121 and every hexagon needing at least one pass, exactly one hexagon receives two passes -- and that hexagon is the fragment, which already had one. Hence **every future ell<5 macro edge is the fragment repair: exactly one when a fragment exists, zero otherwise**, and its departure port, rotation length (5 - c_f) and four target candidates are all determined by the state. Budget census {1: 5947, 0: 710}: no residual state can ever spend Round 85's bound of 2. The counting identity 121 - P == (empty hexagons) + (fragments) holds on all 6,657, an independent confirmation. The 19 were decided by complete port-level enumeration over **every** static slack-cover -- 19/19 UNSAT -- **this subclaim is RETRACTED in Round 90c: the independent result is 15 UNSAT / 4 SAT** -- and a non-MRV independent reimplementation agrees (up to 3.2M nodes, no caps). Re-running Round 88's polynomial joint condition with the corrected budget gives **71 UNSAT, 0 UNKNOWN in 24 s**; minus the 13 already closed in Round 85 that is **58 new closures**. The decisive control: the same pipeline at budget 2 reproduces Round 85/88's 13 **identically by sid**, so the entire gain comes from the theorem and not from a code change. Side correction: slack-cover blocks may overlap inside U, so Rounds 88/89 counted cover *derivations*, not sets -- 19 is a lower bound and 18 of the 19 have only one static cover set at all | this branch, `research/RR_SINGLE_SHORT_PASS_CLAUDE.md`, `outputs/rr_single_short_pass_claude.json` |
| 89 (Claude branch) | **Forced-core dynamics: full census done, closure 0, but 19 states now have a UNIQUE dynamically admissible cover.** Replacing Round 88's 260 per-orbit solver calls (forbid q => UNSAT proves FORCED; force q => UNSAT proves FORBIDDEN) with a single complete enumeration -- FORCED is the intersection of all cover bitmasks and FORBIDDEN is the candidate set minus their union -- decided all **6,657 states with 0 UNKNOWN in 938 s**. **2,637 states (39.6%) carry at least one FORCED orbit** (up to 20), confirming Round 88's sampled 39.7% on the full set; 5,716 states have 40 or more FORBIDDEN candidates and 24 states have exactly one cover. The forced-core source Hall test used the **most generous supplier universe available** -- the open orbits A together with every candidate including OPTIONAL, all five ports each -- precisely so that a state is never closed by ignoring an optional orbit a completion could use as an intermediate; the deficit is **0 for all 2,637**. Two consequences follow rather than being measured: the predecessor/entry cut closes 0, because a deficit of 0 means a complete matching exists and hence every forced target has a supplier; and an optional-support accounting cannot be used soundly until optional orbits are *proved* unusable as intermediates (the same failure mode Round 87's adversarial check caught). On the 1,190 tight states (FORCED >= 1 and at most 20 covers) the Round-88 joint condition was applied to every cover, giving {0: 13, 1: 19, 2-5: 115, 6-20: 1043} with 0 incomplete enumerations; the 13 with none are **identical by sid to Round 85's 13**, so closure is **0** and the residual stays **6,644**. The product is the 19: their final orbit set is determined, so the mechanism that killed Rounds 81-88 -- a different valid cover routing around the bottleneck -- does not exist there. They sit at the budget boundary (short edges used {2: 15, 1: 4}, never 0), all have c in {2,3}, and forced is a subset of the unique cover in 19/19. They are 0.29% of the residual: closing all of them would leave 6,625, and no claim of bulk payoff is made | this branch, `research/RR_FORCED_CORE_CLAUDE.md`, `outputs/rr_forced_core_unique_states.json` |
| 88 (Claude branch) | **Universal cover core exists but closes nothing; closure 0.** Complete enumeration of the cover family shows it is small: of 1,000 sampled states only **one** reached the 2,000 cap and 635 have at most 100 covers. **397 states (39.7%) have at least one FORCED orbit** (up to 17), and 941 have 20 or more FORBIDDEN candidates -- both established by complete finite decisions (q forbidden => UNSAT proves FORCED; q forced => UNSAT proves FORBIDDEN), never by sampling. This **corrects the wording carried since Round 81**: 'enormous cover freedom' was really the candidate-orbit count, whereas the actual solution family is usually under 100. The round then shared the ell<5 short-edge budget of 2 between Round 85's generation exceptions and Round 87's capacity deficits -- strictly stronger than either alone, since the covers passing each test may differ -- and decided it over every valid cover. The 13 UNSAT are **identical by sid to Round 85's 13** (both set differences empty), so capacity contributed **no additional closure**. SAT states consume the budget as {0: 2544, 1: 2846, 2: 1254}, so the constraints are tight and 2 is exactly enough. The wall is not that covers are plentiful -- they are not -- but that one of the hundred always passes everything | this branch, `research/RR_UNIVERSAL_COVER_CLAUDE.md` |
| 87 (Claude branch) | **Cumulative source-port capacity: closure 0, but the resource semantics are proved and genuinely tight.** Every macro edge departs from the walk's endpoint, which is always the previous joint's target and hence a REGISTERED port (engine check 39/39), and no-repeat makes each port an endpoint at most once -- so **each port fires at most one macro edge**, and neither re-entry nor E^1/E^2 can restore a consumed departure. The labelled G5 geometry is dense: exactly 2 targets per source port, **zero target overlap between two ports of the same orbit** (all 1,440 pairs), and exactly 10 supplier orbits per target with one specific port each. Under a generous capacity model 5 of the stored Round-79 witness covers are genuinely unmatchable (deficits {1: 4, 4: 1}), so Hall violations really do exist. But an adversarial check found that ell<5 short edges bypass the matching, and their budget is 2, so the sound closure criterion is deficit >= 3 -- without that correction 4 states would have been closed wrongly. Enumerating all covers leaves **6,657 SAT, 0 UNSAT, 0 UNKNOWN**: the freedom of cover choice always evades the deficient combination, the same cause as every round since 81 | this branch, `research/RR_SOURCE_CAPACITY_CLAUDE.md` |
