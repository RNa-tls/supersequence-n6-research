/* Independent whole-E-block generic one-beta-path necessary exact-P query.
...
static void rec(int start,int length,int b,int D,int a,int q,int R,int H){
 ...
 for(int run=1;run<=5&&length+run<=P;run++){
  ...
  if(length+run==P){
   if(a==amax&&q==qmax&&nd<=dmax){hits++;fprintf(out,"[");for(int j=0;j<P;j++)fprintf(out,"%s%d",j?",":"",path[j]);fprintf(out,"]\n");}
  }else if(nd-4*(bmax-nb)<=dmax){
   for(int j=0;j<nt[u]&&!capped;j++){
    ...
    if(rb<0||rrem<ar+qr)continue;
    int delta=dmax+5*bmax-nd-5*nb;
    if(bound_enabled&&(delta<0||length+run+ub[indexub(ar,qr,rrem,hr,rb,delta)]<P)){suffix_prunes++;continue;}
...
int main(int argc,char**argv){
 ...
 if(bmax<0||bmax>10||dmax<0||dmax>120||amax<0||amax>20||qmax<0||qmax>10||rmax<0||rmax>30||hmax<0||hmax>3||P<1||P>150)fail("invalid arguments");
 ...
 if(argc==11)readbounds(argv[10]);geometry();rec(0,0,0,0,0,0,0,0);if(fclose(out))fail("export close failed");
 printf("{\"schema\":\"round143-general-whole-runs-v1\",\"proof_query\":\"EXACT_P_NOT_CAPACITY\",\"completed\":%s,\"capped\":%s,\"nodes\":%"PRIu64",\"exported_prefixes\":%"PRIu64",\"suffix_bound_enabled\":%s,\"suffix_bound_prunes\":%"PRIu64",\"transcript_fnv64\":\"%016"PRIx64"\"}\n",capped?"false":"true",capped?"true":"false",nodes,hits,bound_enabled?"true":"false",suffix_prunes,digest);
 free(ub);return 0;
}
