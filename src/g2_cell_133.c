...
/* ==== Round 133 instrumentation (compile with -DINSTR; otherwise inert) ==============
   Aggregate-only: histograms by pass depth and a death-reason census.  No raw states are
   ever dumped - the point is to find WHY the search dies, not to record billions of nodes. */
#ifdef INSTR
#define NREASON 24
static const char *RNAME[NREASON] = {
    "cost_bound", "chain_capacity", "shrun_overflow", "d_infeasible", "capacity_ok",
    "fod_deficit", "fout_minimum", "sterile_no_child",
    "succ_costcap", "succ_shcap", "succ_lock_orbit", "succ_xcap", "succ_runlen5",
    "succ_foutcap", "succ_rmax", "succ_shruncap", "succ_repeat_run_rule", "succ_ecap",
    "succ_orbcap", "succ_slot_used", "succ_exccap", "succ_no_pass_option",
    "succ_model", "succ_ordpin"};
static long long inodes[TARGET + 2], idie[NREASON][TARGET + 2], ichild[TARGET + 2];
static long long ireject[NREASON];
static long long iblock_exit_raw = 0, iblock_exit_distinct = 0, isig_full = 0;
#define SIGBITS 22
#define SIGN (1u << SIGBITS)
static uint64_t *sigtab, *sigtab2;
static long long sig_overflow = 0;
static int sig_insert(uint64_t *tab, uint64_t h) {
    if (!h) h = 1;
    uint32_t i = (uint32_t)(h >> 32) & (SIGN - 1);
    for (int probe = 0; probe < 64; probe++) {
        if (tab[i] == 0) { tab[i] = h; return 1; }
        if (tab[i] == h) return 0;
        i = (i + 1) & (SIGN - 1);
    }
    sig_overflow++;
...
#define DIE(r) do { idie[r][passes]++; } while (0)
#define REJ(r) do { ireject[r]++; } while (0)
#else
#define DIE(r) do { } while (0)
#define REJ(r) do { } while (0)
#endif
...
static void dfs(int u, int len, int passes, int cost, int orbits, int runs,
...
#ifdef INSTR
    inodes[passes]++;
    /* the node right after the LAST short pass is the type-B block exit */
    if (passes > 0 && wlen_[passes - 1] < 6 && all_short_placed()) {
        iblock_exit_raw++;
        uint64_t h = mix(u) ^ mix(0x11 * (uint64_t)orbits) ^ mix(0x22 * (uint64_t)runs)
                   ^ mix(0x33 * (uint64_t)cost) ^ mix(0x44 * (uint64_t)shrun)
                   ^ mix(0x55 * (uint64_t)fout) ^ mix(0x66 * (uint64_t)nrev)
                   ^ mix(0x77 * (uint64_t)runlen) ^ mix(0x88 * (uint64_t)passes)
                   ^ mix(0x99 * (uint64_t)EXC);
        for (int q = 0; q < 5; q++) h ^= mix((uint64_t)(q + 1) * 0x1000 + defcnt[q]);
        iblock_exit_distinct += sig_insert(sigtab, h);
        /* a FULL signature additionally fingerprints the used-hexagon set */
        uint64_t hf = h ^ mix(HLO) ^ mix(HHI * 3);
        isig_full += sig_insert(sigtab2, hf);
    }
#endif
    if (passes == TARGET) {
    ...
    }
    {
        ...
        if (rem > 0) {
            ...
            if (extra < 0) extra = 0;
            if (cost + extra > COSTCAP) { DIE(0); return; }
        }
    }
    {
        int sleft = SHRUNCAP - shrun;
        if (sleft < 0) { DIE(2); return; }
        ...
        if (bound < TARGET) { DIE(1); return; }
    }
    if (DCAP >= 0 && !dfeasible(orbid[u], ECAP - nrev)) { DIE(3); return; }
    if (!capacity_ok(orbid[u], ECAP - nrev, passes, orbits)) { DIE(4); return; }
    if (FOD && DCAP >= 0
        && dcommitted(orbid[u], ECAP - nrev) + freshdeficit(ORBCAP - orbits) > DCAP) { DIE(5); return; }
    {
        int isshort = (len < 6);
        int avail = (shorts_total() - shorts_placed()) + (isshort ? 1 : 0);
        if (fout + avail < FOUTMIN) { DIE(6); return; }
    }
...
#ifdef INSTR
    long long kids0 = 0;
#define KID() do { kids0++; } while (0)
#else
#define KID() do { } while (0)
#endif
    ...
    for (int si = silo; si < sihi && !found; si++) {
        ...
        int c = scost[si];
        int hb = shub[si];
        if (cost + c > COSTCAP) { REJ(8); continue; }
        if (SHCAP >= 0 && cost + c + hub + hb > SHCAP) { REJ(9); continue; }
        int nq = orbid[w];
        if (lockorb >= 0 && nq != lockorb) { REJ(10); continue; }      /* case-(i) locality lock */
        int same = (nq == curorb);
        int nruns = runs, nsh = shrun, nrunlen = runlen + 1, nfout = fout;
        int nxj = xj, nnrev = nrev, nhub = hub + hb, nnj = njoint + (hb ? 1 : 0);
        if (same) {
            if (c == 1) { nxj = xj + 1; if (nxj > XCAP) { REJ(11); continue; } }
            if (runlen + 1 > 5) { REJ(12); continue; }
        } else {
            if (c == 0) {
                nfout = fout + 1;
                if (nfout > FOUTCAP) { REJ(13); continue; }
            }
            nruns = runs + 1;
            nsh = shrun + (5 - runlen);
            if (nsh > SHRUNCAP) { REJ(15); continue; }
            if (nruns > RMAX) { REJ(14); continue; }
            nrunlen = 1;
        }
        ...
        if (rv && REVSPEC >= 0) {          /* REVSPEC < 0 disables the rule entirely */
            if (c != 0) continue;
            if (cursid < 0 || !((REVSPEC >> cursid) & 1)) { REJ(16); continue; }
        }
        if (nnrev + rv > ECAP) { REJ(17); continue; }
        if (fresh && orbits + 1 > ORBCAP) { REJ(18); continue; }
        if (omask[nq] >> phse[w] & 1) { REJ(19); continue; }
        int addexc = 0;
        if (fresh) {
            for (int j = 0; j < 5; j++) if (mcnt[ohex[nq][j]]) addexc++;
            if (EXCCAP >= 0 && EXC + addexc > EXCCAP) { REJ(20); continue; }
        ...
        for (int oi = 0; oi < nopt && !found; oi++) {
            ...
            if (MTYPE != 0 && kind == 2 && slot == 1 && bov[0] >= 0) {
                ...
                if (LOCK0MODE == 0 && (q0orb < 0 || nq != q0orb)) { REJ(22); continue; }
                /* Model T needs the two openers to share an orbit; D-alpha forbids it. */
                if (LOCK0MODE == 3 && nq != orbid[bov[0]]) { REJ(22); continue; }
                if (LOCK0MODE == 1 && nq == orbid[bov[0]]) { REJ(22); continue; }
            }
            /* ---- Round 132: proved walk-order chains ------------------------------- */
            if (ORDPIN == 1 && MTYPE != 0) {          /* alpha chain */
                if (kind == 3 && bpos[slot] >= 0 && passes != bpos[slot] + 5) { REJ(23); continue; }
                if (kind == 2 && slot == 1 && bpos[0] >= 0 && passes <= bpos[0] + 5) { REJ(23); continue; }
            } else if (ORDPIN == 2 && MTYPE != 0) {   /* beta nest */
                if (kind == 2 && slot == 1 && bpos[0] >= 0
                    && (passes < bpos[0] + 1 || passes > bpos[0] + 4)) { REJ(23); continue; }
                if (kind == 3 && slot == 1 && bpos[1] >= 0 && passes != bpos[1] + 5) { REJ(23); continue; }
                if (kind == 3 && slot == 0 && bpos[0] >= 0 && passes != bpos[0] + 10) { REJ(23); continue; }
            }
...
#ifdef INSTR
    sigtab  = calloc(SIGN, sizeof(uint64_t));
    sigtab2 = calloc(SIGN, sizeof(uint64_t));
    if (!sigtab || !sigtab2) { fprintf(stderr, "instr alloc failed\n"); return 4; }
#endif
...
#ifdef INSTR
    {
        fprintf(stderr, "{\"instr\": 1, \"target\": %d, \"max_depth\": %d,\n", TARGET, bestPasses);
        fprintf(stderr, " \"block_exit_raw\": %lld, \"block_exit_distinct_coarse\": %lld,"
                        " \"block_exit_distinct_full\": %lld, \"sig_overflow\": %lld,\n",
                iblock_exit_raw, iblock_exit_distinct, isig_full, sig_overflow);
        fprintf(stderr, " \"reject_totals\": {");
        for (int r = 8; r < NREASON; r++)
            fprintf(stderr, "%s\"%s\": %lld", r > 8 ? ", " : "", RNAME[r], ireject[r]);
        fprintf(stderr, "},\n \"by_depth\": [\n");
        for (int d = 0; d <= TARGET; d++) {
            long long tot = inodes[d];
            if (!tot) continue;
            fprintf(stderr, "  {\"depth\": %d, \"nodes\": %lld, \"children\": %lld", d, tot, ichild[d]);
            for (int r = 0; r < 8; r++)
                if (idie[r][d]) fprintf(stderr, ", \"%s\": %lld", RNAME[r], idie[r][d]);
            fprintf(stderr, "}%s\n", d < TARGET ? "," : "");
        }
        fprintf(stderr, " ]}\n");
    }
