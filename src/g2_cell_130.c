/* 라운드 130 — 일반 `(k,G) = (4,2)` 칸을 끝까지 도는 정확 solver.
 *
...
static int bo, bc, bpend[2], bplen[2];
...
static void dfs(int u, int len, int passes, int cost, int orbits, int runs,
                ...
                int lockorb, int lockwait, int pend) {
    if (found) return;
    ...
    for (int si = silo; si < sihi && !found; si++) {
        ...
        for (int oi = 0; oi < nopt && !found; oi++) {
            ...
            else if (kind == 2) { bpend[slot] = SIG[nlen][w]; bplen[slot] = 6 - nlen; bo++; }
            else if (kind == 3) { bpend[slot] = -1; bc++; }
            int sid = (kind == 1) ? slot : ((kind == 2) ? 2 * slot : 2 * slot + 1);
            int cf = (kind == 0) ? 0
                     : (!FREEON ? 0 : ((FREESPEC >> sid & 1) ? 1 : 2));
            int nlo = lockorb, nlw = lockwait;
            if (pend >= 0) { nlo = nq; nlw = nusid(pend); }
...
int main(int argc, char **argv) {
    ...
    for (int fi = 0; fi < nfirst && !found; fi++) {
        ...
        if (kind == 2) { bpend[0] = SIG[len][start]; bplen[0] = 6 - len; bo = 1; }
    ...
    printf("{\"mtype\": %d, \"orbcap\": %d, \"costcap\": %d, \"xcap\": %d, \"foutcap\": %d,"
           ...
           FREESPEC, FREEON, LOCKSPEC, SHRUNCAP,
