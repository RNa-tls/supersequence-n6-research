...
def zero_cost_components(nodes, edges, cost):
    ...
    **전제 (라운드 105):** 각 정점의 비용-0 유출 차수는 ≤ 1 이어야 한다.  이 전제가 없으면
    `c − 1` 하한은 일반 유향 그래프에서 거짓이다 (라운드 105 §8 이 반례 111건을 찾았다).
    """
...
def weighted_hamilton(nodes, edges, cost, B, node_cap=200_000):
    ...
    def dfs(cur, rem, spent):
        ...
        for i in idxs:
            t = out0[i] & rem
            if t:
                # 전제: 비용-0 유출 차수 ≤ 1.  라운드 105 의 반증 탐색이 밝혔듯이 이 전제가
                # 깨지면 성분 수가 과대계수되어 하한이 **불건전**해진다 (일반 0/1 유향
                # 그래프에서 실제 반례 111건).  실제 기하에서는 `T1` 이 유일하므로 성립하고,
                # 173,409 배정 전수에서 최대 유출 차수 1 로 확인했다.
                assert t & (t - 1) == 0, "비용-0 유출 차수 > 1 — 성분 하한 전제 위반"
                j = t.bit_length() - 1
                a, b2 = find(i), find(j)
                if a != b2:
                    par[a] = b2
