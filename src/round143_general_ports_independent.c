/* Round 143 independent generic one-beta-path necessary relaxation.
 *
...
static void visit(int current,Counts c) {
    ...
    if(c.p==PMAX) {
        if(c.a==AMAX && c.q==QMAX && c.deficit<=DMAX)export_path(c);
        return;
    }
    ...
    for(int i=0;i<nedges[current] && !capped;i++) {
        ...
        if(bound_mode && e->kind!=K_E) {
            ...
            int bound=suffix_bound(AMAX-next.a,QMAX-next.q,RMAX-next.repeats,
                HMAX-next.heavy,BMAX-next.b,delta);
...
int main(int argc,char **argv) {
    ...
    BMAX=parse_int(argv[1],719);DMAX=parse_int(argv[2],720);
    char *end;errno=0;limit=strtoull(argv[3],&end,10);
    if(errno || end==argv[3] || *end || argv[3][0]=='-')die("Invalid node cap");
    AMAX=parse_int(argv[4],719);QMAX=parse_int(argv[5],719);
    RMAX=parse_int(argv[6],719);HMAX=parse_int(argv[7],2157);PMAX=parse_int(argv[8],720);
    if(!PMAX)die("P must be positive");
    ...
    printf("{\"schema\":\"round143-general-independent-port-v1\",\"proof_query\":\"EXACT_P_NOT_CAPACITY\","
