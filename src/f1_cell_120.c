...
static int ohex[NO][5];        /* the 5 distinct hexagons each orbit meets */
...
static void dfs(int u, int len, int passes, int cost, int orbits, int runs,
    ...
    int hregion_ok = (HREGION == 0) || (HREGION == 1 && sstate == 0)
                     || (HREGION == 2 && sstate == 2);
    if (hub < HCAP && !forcefree && hregion_ok)
        for (int h = 0; h < NH4; h++) {
...
int main(int argc, char **argv) {
    ...
    NODECAP = (argc > 20) ? atoll(argv[20]) : 200000000000LL;
