...
def main():
    ...
    result=dict(schema='codex/g2-k4-locked-detour-closure/1',
                ...
                capacity_source_lf_sha256=hashlib.sha256(source.read_bytes().replace(b'\r\n',b'\n')).hexdigest(),
                capacity_source_git_blob=subprocess.check_output(['git','rev-parse','HEAD:src/chain_capacity_115.c'],cwd=ROOT,text=True).strip(),
                line_ending_policy='original source raw SHA retained; portable C check normalizes CRLF to LF only; new artifact bytes protected by .gitattributes',
