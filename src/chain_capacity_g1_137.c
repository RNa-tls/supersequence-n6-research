/* 독립 라운드 — **한 겹친 육각형(G=1)** 을 허용하는 사슬 용량 전수 탐색기 N1*(b,g,s).
 *
 * 라운드 115 의 F=0 사슬 모델(chain_capacity_115.c)에 짧은 pass 쌍을 하나 더한 것이다.
 * 짧은 pass 의 의미론은 이 라운드의 정리 I·II 에서 **정확히** 온다:
 *   * 진입 u, 길이 L(1..5) 인 짧은 pass 는 슬롯 (orb(u), phase(u)) 과 육각형 hex(u) 를 쓴다;
 *   * 그 nu-목표는 c = sigma^L(u) 이고 닫는 pass 는 (c, 6-L) 로 **같은 육각형**을 다시 쓴다;
 *   * 이어지는 이동은  M2 -> orb(c) 의 phase(c)+1,  M3a -> orb(c) 의 phase(c)+2,
 *     M3b/M3c -> 제3의 궤도 (탈출 단어 sigma^{L-1}(u) 에서 계산).
 * 원본 모델과 마찬가지로 run 확장은 현재 궤도의 임의의 미사용 phase 로 가고 (+1 이면 무료,
 * 아니면 b 에서 1) 이는 현실의 완화이므로 결과는 **참된 상한**이다.
 *
 * 원래 헤더:
...
static int SIGP[7][NW];          /* SIGP[k][w] = sigma^k(w) */
static int sM3b[NW], sM3c[NW];   /* M3b/M3c computed on an ARBITRARY exit word */
...
static void build(void) {
    ...
    /* sigma powers, and the light connectors evaluated on an arbitrary exit word */
    for (int w = 0; w < NW; w++) {
        int y[6]; memcpy(y, perm[w], sizeof y);
        SIGP[0][w] = w;
        for (int k = 1; k <= 6; k++) { int o[6]; sig_(y, o); memcpy(y, o, sizeof y);
                                       SIGP[k][w] = rank_of(y); }
        int *q = perm[w];
        int b2[6] = {q[3], q[4], q[5], q[2], q[0], q[1]};
        int c3[6] = {q[3], q[4], q[5], q[2], q[1], q[0]};
        sM3b[w] = rank_of(b2);
        sM3c[w] = rank_of(c3);
    }
}
...
static int SHORT_ON;                 /* 1 = a doubled hexagon is allowed */
static int pend_closer;              /* pending closer word, -1 = none */
static int pend_len;                 /* its length */
static int short_state;              /* 0 = unopened, 1 = pending, 2 = done */
...
static void dfs(int cur, int corb, int bused, int passes, int nruns) {
    ...
    if (capped) return;
    if (feasible(GCAP, -1) && (!SHORT_ON || short_state == 2)) {
    ...
    /* (1b) OPEN the doubled hexagon here: re-read the CURRENT pass as a short pass.
       A short pass of length L at entry `cur` keeps the same slot and hexagon, but its
       continuation is redirected to orb(sigma^L(cur)) - Theorems I and II. */
    if (SHORT_ON && short_state == 0) {
        for (int L = 1; L <= 5; L++) {
            int c = SIGP[L][cur];
            int y = SIGP[L - 1][cur];            /* exit word of the short pass */
            int nq = orbid[c];
            /* the four continuations */
            int cand[4];
            cand[0] = word_at[nq][(phse[c] + 1) % 5];    /* M2  : nu-target phase + 1 */
            cand[1] = word_at[nq][(phse[c] + 2) % 5];    /* M3a : nu-target phase + 2 */
            cand[2] = sM3b[y];
            cand[3] = sM3c[y];
            for (int ci = 0; ci < 4; ci++) {
                ...
                if ((HLO & hlo[w]) || (HHI & hhi[w])) continue;
                int q2 = orbid[w];
                int fresh = (omask[q2] == 0);
                int nb = bused + (fresh ? 0 : 1);
                if (nb > BCAP) continue;
                if (omask[q2] >> phse[w] & 1) continue;
                short_state = 1; pend_closer = c; pend_len = 6 - L;
                if (fresh) { ntouch++; defcnt[4]++; }
                else { int d0 = 5 - __builtin_popcount(omask[q2]);
                       defcnt[d0]--; defcnt[d0 - 1]++; }
                omask[q2] |= 1 << phse[w];
                HLO |= hlo[w]; HHI |= hhi[w];
                dfs(w, q2, nb, passes + 1, nruns + 1);
                HLO &= ~hlo[w]; HHI &= ~hhi[w];
                omask[q2] &= ~(1 << phse[w]);
                if (fresh) { ntouch--; defcnt[4]--; }
                else { int d0 = 5 - __builtin_popcount(omask[q2]);
                       defcnt[d0 - 1]--; defcnt[d0]++; }
                short_state = 0; pend_closer = -1;
            }
        }
    }
    /* (1c) CLOSE the doubled hexagon: the pending closer word reuses its hexagon. */
    if (SHORT_ON && short_state == 1) {
        int w = pend_closer;
        int q2 = orbid[w];
        if (!(omask[q2] >> phse[w] & 1)) {
            int same = (q2 == corb);
            int fresh = (omask[q2] == 0);
            int extra, ok = 1;
            if (same) { extra = (phse[w] == (phse[cur] + 1) % 5) ? 0 : 1; }
            else      { extra = fresh ? 0 : 1; }
            if (bused + extra > BCAP) ok = 0;
            if (ok) {
                short_state = 2;
                if (!same && fresh) { ntouch++; defcnt[4]++; }
                else { int d0 = 5 - __builtin_popcount(omask[q2]);
                       defcnt[d0]--; defcnt[d0 - 1]++; }
                omask[q2] |= 1 << phse[w];
                dfs(w, q2, bused + extra, passes + 1, nruns + (same ? 0 : 1));
                omask[q2] &= ~(1 << phse[w]);
                if (!same && fresh) { ntouch--; defcnt[4]--; }
                else { int d0 = 5 - __builtin_popcount(omask[q2]);
                       defcnt[d0 - 1]--; defcnt[d0]++; }
...
int main(int argc, char **argv) {
    if (argc < 4) { fprintf(stderr, "usage: %s b g s [nodecap] [short_on]\n", argv[0]); return 1; }
    build();
    BCAP = atoi(argv[1]); GCAP = atoi(argv[2]); SCAP = atoi(argv[3]);
    NODECAP = (argc > 4) ? atoll(argv[4]) : 20000000000LL;
    SHORT_ON = (argc > 5) ? atoi(argv[5]) : 1;
    pend_closer = -1; pend_len = 0; short_state = 0;
