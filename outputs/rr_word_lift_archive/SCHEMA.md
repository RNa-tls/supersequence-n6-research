...
## 라운드 101 추가: `wc_wd.jsonl.gz`

W-A 를 통과한 **15,781 쌍**에 대한 단어 수준 국소 일관성 판정.

| 층 | 규칙 | 근거 |
|---|---|---|
| **W-C (P)** | 전임자 지지가 없는 후보 단어 삭제 | 완성 경로에서 의무는 정확히 한 번, ROOT 또는 다른 의무에서 진입된다 |
| **W-C (S)** | 후속이 전혀 없는 의무가 2개 이상이면 모순 | terminal 은 전역에서 하나 |
| **W-C (R)** | 살아 있는 도메인만으로 재도달성 | 경로는 ROOT 에서 이어진다 |
| **W-C (E)** | 빈 도메인 | 진입 단어가 없다 |
| **W-D** | 각 의무를 서로 다른 전임자가 공급하는 완전 매칭 | 의무는 정확히 한 번 발사한다 (라운드 93 Hall 의 단어 수준 정련) |

행 필드: `sid`, `cover_id`, `obligations`, `domain_sizes`, `ac_deletions`, `pair_verdict`,
그리고 실패 시 `certificate`.

**결과: 쌍 실패 5(전부 W-C (S)), 상태 폐쇄 0.** 단어 수준 국소 일관성은 W-A 위에서 사실상
소진됐다.

`probe_rr_word_csp.py` 자체가 표준 라이브러리만 쓰고 **순열 문자열에서 직접** 기하를
계산하므로 이 파일의 재현에는 별도 검증기가 필요 없다.
