...
### 남은 것은 손증명이 아니다 — 기계 잔여 두 가지

**(R1) 단일 구현으로만 인증된 용량 셀** (`r163/certs/coverage_163.json`).

| | C 검사기 | 파이썬 검사기 | 이중 구현 | **단일 구현** | 공유 셀 불일치 |
|---|---:|---:|---:|---:|---:|
| 사슬 용량 | 1,101 | 854 | 854 | **247** | 0 |
| 조각 용량 | 220 | 111 | 111 | **109** | 0 |

파이썬 검사기는 노드 상한 2,000 만으로 돌고 C 검사기는 최대 400 억으로 돈다
(실제 총 노드 835 억 + 1,030 억).  그래서 **비싼 셀일수록 한 구현에만
의존**한다.  인구조사는 1,101 셀을 **전부** 읽고 미사용 셀이 0 이므로,
이 356 개 셀은 전부 적재된다.  다만 `checker152.py` 는 생산 solver 와
**독립**이다 — 카탈로그를 문자열 대수로 다시 세우고, 가지치기 정리를
진술에서 구현하며, 생산 표를 읽지 않고, 노드 상한에 닿으면
`UNKNOWN_CAP` 으로 **실패-닫힘**한다.  그리고 이중 구현된 965 셀에서
용량 불일치는 **0** 이다.
...
| 항목 | 내용 |
|---|---|
| 무엇 | 검사기의 마스크·카운트·토큰이 정말 보조정리가 말하는 `(u_q, k)` 인가 |
| 왜 부족한가 | 보조정리는 증명되었고 (r150 §2), 알고리즘 동일성은 운용 정의역 220,255 상태에서 **전수** 확인되었으며 (r163), C 구현 5 개의 대응은 179,850 사례에서 불일치 0 이다.  그러나 상태 유지 자체는 **논증**이고 역사적 per-prune 기록은 존재하지 않는다 (`no_per_prune_trace_available: true`) |
| 완화 | `checker152.py` 가 생산 solver 와 독립이므로 라운드 150 이 말한 "생산 상태 유지" 위험은 **줄어들었지 사라지지는 않았다** — 검사기 자신의 상태 유지로 옮겨갔을 뿐이다 |
| 하위 영향 | `C.chaincaps`, `C.piececaps` → `C.census`, `E.enumeration` → 사실상 경로 전체 |
| 성격 | **기계 충실성** 의무이지 손증명 의무가 아니다.  라운드 150 자신이 `cheapest_remaining_risk` 로 적어 두었다 |

### 라운드 164 권고 — **단일 구현 셀 356 개의 독립 재인증**

미해결 손증명이 없으므로 §16 에 따라 **질적으로 다른 공격**을 권고한다.
저장소가 실제로 지탱하는 가장 구체적인 것은 다음 하나다.

> **`verify_all_c152.json` 의 247 개 사슬 셀과 `verify_piece_c152.json` 의
> 109 개 조각 셀 — 인구조사가 읽지만 한 구현에만 의존하는 셀 — 을
> 두 번째 독립 구현으로 재인증한다.  그 과정에서 feas 를 쓰되,
> 라운드 163 이 운용 정의역 전수로 고정한 형태
> (히스토그램 `(n1,n2,n3,n4)` + 토큰 → 부분집합 최적) 를 그대로 쓰고,
> 검사기의 상태 유지가 그 히스토그램을 옳게 만드는지 각 노드에서
> 검사(assert)한다.**

근거: (i) 남은 두 잔여가 **같은 356 개 셀**에서 만난다 — 이중 구현이 없는
셀이 곧 상태 유지가 교차검증되지 않는 셀이다; (ii) 저장소에 이미 독립
검사기 두 개(`r152/src/checker152.py` + C 쌍둥이)와
`r149/src/certcheck149.py` 가 있어 세 번째를 쓸 출발점이 있다;
(iii) 비용이 예측 가능하다 — 파이썬 검사기가 2,000 만 노드로 854 셀을
2,841 초에 처리했으므로, 남은 셀은 노드 상한을 올리는 문제이지 새 수학이
아니다; (iv) 라운드 150 이 "route A: 보편 증명 + 구현 대응" 이라고
명시했으므로 **route B(인증서 재생)는 아직 아무도 하지 않은 일**이다.

형식화·외부 재현·논문화는 그 다음이다.  지금은 증명의 **가장 싼 실패
가능성**이 356 개 셀로 좁혀져 있고, 그 356 개를 겨냥하는 것이 옳다.
...
python3 r163/src/coverage163.py
...
| `coverage163.py` | `coverage_163.json` **바이트 동일** (저장 인증서만 읽는다) |
...
**라운드 164 권고**: 인구조사가 읽지만 **단일 구현으로만 인증된 356 개 셀**
(사슬 247 + 조각 109) 을 두 번째 독립 구현으로 재인증하고, 그 검사기가 각
노드에서 feas 히스토그램의 상태 유지를 assert 하게 한다.  남은 두 기계
잔여가 정확히 이 356 개 셀에서 만난다.
